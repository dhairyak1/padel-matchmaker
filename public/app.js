...
async function loadVenues() {
  ...
  sortedVenues.forEach((venue) => {
    ...
    item.onclick = () => {
      ...
      document.getElementById("venuesModal").style.display = "none";
    };
  ...
  venueSearch.addEventListener("input", function () {
    const search = venueSearch.value.toLowerCase().trim();
...
if (hostForm) {
  ...
  if (successButton) {
    successButton.addEventListener("click", () => {
      window.location = "/my-matches.html";
    });
  }
  ...
  hostForm.addEventListener("submit", async function (e) {
    ...
    if (response.ok) {
      ...
      submitButton.textContent = "Publish Match";
...
if (showVenuesBtn) {
  showVenuesBtn.addEventListener("click", () => {
    document.getElementById("venuesModal").style.display = "flex";
  });
}
